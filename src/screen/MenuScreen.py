...
from screen.MenuItems import TableCreator
import screen.LevelScreen, screen.OptionScreen, screen.InstructionScreen, screen.EndScreen
...
class MenuScreen(BaseMenuScreen):
    ...
    def __init__(self, game):
        ...
        self.mButtonTable = TableCreator(b2Vec2(self.modelsize.x, self.modelsize.y + 2), 1, 5,
                                         ...
                                         [lambda: self.mGame.setScreen(screen.LevelScreen.LevelScreen(self.mGame)),
                                           lambda: self.mGame.setScreen(screen.OptionScreen.OptionScreen(self.mGame)),
                                           lambda: self.mGame.setScreen(screen.InstructionScreen.InstructionScreen(self.mGame)),
                                           lambda: self.mGame.setScreen(screen.EndScreen.EndScreen(self.mGame)),
                                           lambda: Pgl.app.stop()])
